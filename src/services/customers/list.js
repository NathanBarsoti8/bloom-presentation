...
module.exports = async (req, res) => {
    ...
    if (req.query.isActive == 'false')
        params.push({ isActive: 0 })
    else
        params.push({ isActive: 1 })
    
    if (req.query.name) 
        params.push({ name: { [Op.like]: `%${req.query.name}%`} })

    if (req.query.phoneNumber)
        paramsPhones.push({ number: { [Op.like]: `%${req.query.phoneNumber}%`} })
    ...
    const customers = await Customers.findAndCountAll({
        ...
        include: [{
            ...
            required: paramsPhones.length > 0 ? true : false,
            where: {
                [Op.and]: paramsPhones
            }
        }],
